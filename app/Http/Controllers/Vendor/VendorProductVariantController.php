...
class VendorProductVariantController extends Controller
{
    ...
    {
        $product = Product::where('vendor_id', auth()->id())->findOrFail($product_id);
    ...
    public function store(Request $request, $product_id)
    {
        ...
        $request->merge([
            'is_default' => $request->exists('is_default') ? filter_var($request->input('is_default'), FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE) : null,
            'is_active'  => $request->exists('is_active') ? filter_var($request->input('is_active'), FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE) : null,
            'is_featured'=> $request->exists('is_featured') ? filter_var($request->input('is_featured'), FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE) : null,
        ]);
 ...
 dd($request->all());
    ...
    {
        $product = Product::where('vendor_id', auth()->id())->findOrFail($product_id);
        $variant = ProductVariant::where('product_id', $product_id)->findOrFail($variant_id);
