...
<div class="page-header">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
        ...
        <div class="page-header-right d-flex align-items-center gap-2">
            <a href="{{ route('employee-management.leave.create') }}" class="btn btn-sm btn-primary">
...
<div class="">
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center py-2">
            <h5 class="card-title mb-0">Leave List</h5>
        </div>
...
$(document).ready(function() {
    const table = $('#leaveTable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{{ route("employee-management.leave.data") }}',
            data: function(d) {
        ...
        columns: [
            ...
            { data: 'status', orderable: false },
            {
                ...
                render: function(data, type, row) {
                    ...
                    const id = row.id;
                    const status = row.status_value || 'pending';
                    const editUrl = '{{ route("employee-management.leave.edit", ":id") }}'.replace(':id', id);
                    const deleteUrl = '{{ route("employee-management.leave.destroy", ":id") }}'.replace(':id', id);
                    const statusUrl = '{{ route("employee-management.leave.status", ":id") }}'.replace(':id', id);
                    const csrfToken = '{{ csrf_token() }}';
                    
                    let statusButtons = '';
                    if (status === 'pending') {
                        statusButtons = '<a href="javascript:void(0);" class="btn btn-sm btn-link text-success p-1" onclick="changeStatus(\'' + id + '\', \'approved\', \'' + statusUrl + '\')" title="Approve"><i class="bi bi-check-circle"></i></a>' +
                                       '<a href="javascript:void(0);" class="btn btn-sm btn-link text-danger p-1" onclick="changeStatus(\'' + id + '\', \'rejected\', \'' + statusUrl + '\')" title="Reject"><i class="bi bi-x-circle"></i></a>';
                    }
