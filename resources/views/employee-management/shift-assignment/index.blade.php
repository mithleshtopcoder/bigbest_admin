...
<div class="page-header">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
        <div class="page-header-left d-flex" style="align-items: baseline;">
            <h1 class="page-title mb-0">Shift Assignment</h1>
            <nav aria-label="breadcrumb" class="px-2">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('home') }}" class="text-decoration-none">Home</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Shift Assignment</li>
                </ol>
            </nav>
        </div>
        <div class="page-header-right d-flex align-items-center gap-2">
            <a href="{{ route('employee-management.shift-assignment.create') }}" class="btn btn-sm btn-primary">
                <i class="bi bi-plus-circle me-2"></i>Assign Shift
            </a>
        </div>
    </div>
</div>
...
<div class="">
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center py-2">
            <h5 class="card-title mb-0">Shift Assignment List</h5>
        </div>
        <div class="card-body p-2">
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-3">
                    <select class="form-select form-select-sm" id="shiftFilter">
                        ...
                        @foreach(\App\Models\Shift::where('is_active', true)->orderBy('start_time')->get() as $shift)
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0" id="shiftAssignmentTable">
                    <thead>
                        <tr class="border-b">
                            <th scope="col">No</th>
...
$(document).ready(function() {
    const table = $('#shiftAssignmentTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: '{{ route("employee-management.shift-assignment.data") }}',
            data: function(d) {
                d.search = $('#searchInput').val();
                d.employee_id = $('#employeeFilter').val();
                d.shift_id = $('#shiftFilter').val();
        ...
        columns: [
            ...
            { data: 'is_active', orderable: false },
            {
                ...
                render: function(data, type, row) {
                    ...
                    const editUrl = '{{ route("employee-management.shift-assignment.edit", ":id") }}'.replace(':id', id);
                    const deleteUrl = '{{ route("employee-management.shift-assignment.destroy", ":id") }}'.replace(':id', id);
                    const statusUrl = '{{ route("employee-management.shift-assignment.status", ":id") }}'.replace(':id', id);
    ...
    $('#searchInput, #employeeFilter, #shiftFilter, #statusFilter').on('change keyup', function() {
        table.draw();
    });
});
...
function changeStatus(id, status, url) {
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                $('#shiftAssignmentTable').DataTable().draw();
                alert('Shift assignment status updated successfully');
            } else {
