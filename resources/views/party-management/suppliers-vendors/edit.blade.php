...
<div class="">
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center py-2">
            <h5 class="card-title mb-0">Edit {{ ucfirst($type) }}</h5>
        ...
        <div class="card-body p-2">
            <form action="{{ route('party-management.suppliers-vendors.update', ['type' => $type, 'id' => $supplier->id]) }}" method="POST" id="supplierForm">
                @csrf
                @method('PUT')
                <div class="row row-p">
                    <div class="col-12">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-4 ps-1">
                                <input type="text" class="form-control @error('pincode') is-invalid @enderror" id="pincode" name="pincode" value="{{ old('pincode', $supplier->pincode) }}">
                                @error('pincode')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-4 ps-1">
                                <input type="text" class="form-control @error('country') is-invalid @enderror" id="country" name="country" value="{{ old('country', $supplier->country ?? 'India') }}">
                                @error('country')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-4 ps-1">
                                <input type="text" class="form-control @error('gst_number') is-invalid @enderror" id="gst_number" name="gst_number" value="{{ old('gst_number', $supplier->gst_number) }}" maxlength="15">
                                @error('gst_number')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-4 ps-1">
                                <input type="text" class="form-control @error('pan_number') is-invalid @enderror" id="pan_number" name="pan_number" value="{{ old('pan_number', $supplier->pan_number) }}" maxlength="10">
                                @error('pan_number')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-10 ps-1">
                                <textarea class="form-control @error('payment_terms') is-invalid @enderror" id="payment_terms" name="payment_terms" rows="3" placeholder="e.g., Net 30, COD, Advance payment required">{{ old('payment_terms', $supplier->payment_terms) }}</textarea>
                                @error('payment_terms')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
        ...
        <div class="card-footer py-2">
            <div class="d-flex justify-content-end gap-2">
                ...
                <button type="submit" form="supplierForm" class="btn btn-sm btn-primary">
                    <i class="bi bi-save me-2"></i>Update {{ ucfirst($type) }}
                </button>
            </div>
        </div>
    </div>
